...
def init_db():
    ...
    _init_pending_photos_table()
    _start_sync_worker()


# =============================================================================
#  PENDING PHOTOS  —  remote (e.g. Render) photo uploads waiting for a
#  machine with the face model (the Jetson) to embed them.
#
#  Render can't run InsightFace (not enough memory), so when an admin
#  uploads a photo there, portal.py saves it to Render's own (ephemeral,
#  Jetson-inaccessible) disk and ALSO queues the raw bytes here. This table
#  is the hand-off point: entry_cameras.py polls it, decodes each photo,
#  runs it through its own already-loaded model, saves a local copy under
#  PHOTOS_DIR, and marks the row processed. Photos here are small (profile
#  pics, tens of KB), so storing raw bytes in Postgres is simpler than
#  standing up a separate object-storage bucket for this volume.
# =============================================================================
def _init_pending_photos_table():
    try:
        conn = get_conn()
        try:
            cur = conn.cursor()
        ...
        print(f"[db] pending_photos table init skipped (offline?): {e}")


def queue_pending_photo(employee_id, emp_name, filename, photo_bytes, is_profile=False):
    """Called from portal.py (Render side) when a photo is uploaded but
       this machine has no face model to embed it with."""
    conn = get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
            """INSERT INTO pending_photos
               (employee_id, emp_name, filename, photo_bytes, is_profile)
               VALUES (%s, %s, %s, %s, %s)""",
            (employee_id, emp_name, filename, psycopg2.Binary(photo_bytes), is_profile),
        )
        conn.commit()
        cur.close()
    finally:
...
def fetch_pending_photos(limit=20):
    """Called from entry_cameras.py (Jetson side) to pick up queued photos."""
    conn = get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
            """SELECT id, employee_id, emp_name, filename, photo_bytes, is_profile
               FROM pending_photos
...
def mark_photo_processed(photo_id):
    conn = get_conn()
    try:
        cur = conn.cursor()
        cur.execute(
            "UPDATE pending_photos SET processed = TRUE, processed_at = NOW() WHERE id = %s",
            (photo_id,),
        )
        conn.commit()
        cur.close()
