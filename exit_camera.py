...
def _save_capture(frame, bbox, name, cam_id):
    ...
    try:
        ...
        # Background mein Supabase pe upload karo (non-blocking)
        def _upload_to_supabase(local_path, storage_path):
            try:
                import requests

                SUPABASE_URL = "https://bjgiirtpqfjaaxnftptf.supabase.co"
                SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6ImJqZ2lpcnRwcWZqYWF4bnR0cHRmIiwicm9sIjoiYW5vbiIsImlhdCI6MTc4NTk5Mzk1MywiZXhwIjoyMTAxNTY5OTUxM30.-HBEfhHPJ6KhFUwhGad90mWLiJzQe7Ylw2pMRhxVFaU"
                ...
                if response.ok:
                    print(f"[photo] Supabase upload successful: {storage_path}")
                else:
                    print(
                        f"[photo] Supabase upload failed: "
